...
def prepocces_file(file, sent_tok=True):
	...
	with open(os.path.join(db_path, file), encoding='utf-8-sig') as f:
		text = f.read()
		text = re.sub('\d+,?.?\d*', "", text)	# remove all digits
		text = re.sub(r' +', ' ', text) # remove all whitespaces
		...
		if sent_tok == False:
			...
			tokens = [stemmer.stem(word.lower()) for word in word_tokenize(text) if not word.lower() in _words_to_remove and not if_entity(word)]
			...
			tokens = [[stemmer.stem(word.lower()) for word in word_tokenize(sentence) if not any_in(['.', '?', '!'], word) and not (word.lower() in _words_to_remove)\
